...
export function Hero3Section() {
  ...
  const scale          = useTransform(scrollYProgress, [0, 0.72], [1, 6])
  const labelOpacity   = useTransform(scrollYProgress, [0, 0.18], [1, 0])
  const wrapperOpacity = useTransform(scrollYProgress, [0.85, 1.0], [1, 0])

  // Option 4 — Next Section Rises Through the Letters
  // Step 1: background + shapes fade out as zoom completes → letters become a transparent portal
  const bgOpacity   = useTransform(scrollYProgress, [0.72, 0.84], [1, 0])
  // Step 2: next-section gradient fades in behind the letters (portal peek)
  const peekOpacity = useTransform(scrollYProgress, [0.72, 0.90], [0, 1])
  // Step 3: the whole hero rises upward + fades, revealing OurBusinesses below
  const heroY       = useTransform(scrollYProgress, [0.80, 1.0], reduce ? [0, 0] : [0, -160])
  ...
  useEffect(() => {
    ...
    function drawFrame() {
      ...
      // Compute and cache font-size once per canvas dimension
      if (!fontCache) {
        let fs = h * 0.82
      ...
      if (video.readyState >= 2) {
        ...
        // Clip video to letter shapes
        ctx.globalCompositeOperation = 'destination-in'
        ctx.fillStyle = 'black'
        ctx.fillText('Eloma', w / 2, h / 2)
        ctx.globalCompositeOperation = 'source-over'
      } else {
        // Fallback while video loads — solid navy text so canvas is never blank
  ...
  return (
    <>
      <style>{`
        #hero3-section { height: 300vh; position: relative; background: #EEF3F0; }
      ...
      <div id="hero3-section" ref={sectionRef}>
        ...
        >
          ...
          {/* Background layer: BG colour + vignette + floating shapes — all fade out after zoom */}
          ...
          {/* Portal peek: next-section gradient fades in behind the letters */}
          <motion.div aria-hidden style={{
            position: 'absolute', inset: 0, zIndex: 0,
            pointerEvents: 'none',
            background: 'linear-gradient(180deg, #FBFBF9 0%, #EEF3F0 100%)',
            opacity: peekOpacity,
            willChange: 'opacity',
          }} />
          ...
          <motion.div
            ...
            style={{
              ...
              left: 0, right: 0, zIndex: 2, pointerEvents: 'none',
            }}
          >
            <motion.p style={{
          ...
          <video ref={videoRef} muted loop playsInline preload="auto" style={{ display: 'none' }}>
